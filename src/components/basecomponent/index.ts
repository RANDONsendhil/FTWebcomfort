import { createGlobalTemplate } from "../basecomponent/template/base.html";

/**
 * Base class for web components with toggle functionality and text updates.
 * @template T - Configuration type for the component.
 */
export abstract class FTWebconfortBaseComponent<T extends { name: string; description: string; template: string }> {
	...
	private initializeTemplate(): void {
		this.$container.innerHTML = createGlobalTemplate(this.config.name, this.config.template, this.config.description);
	}
	...
	protected updateText(): void {
		...
		const statusText = this.active ? "Personnalisation : Activée" : "Personnalisation : Désactivée";
